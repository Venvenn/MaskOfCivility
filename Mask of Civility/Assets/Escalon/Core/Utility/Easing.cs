...
{
    /// <summary>
    /// Implementation of the easings from https://easings.net/
    /// </summary>
    public sealed class Easing
    {
        ...
        public static Vector4 Ease(Shape shape, Extent extent, Vector4 a, Vector4 b, float t)
        {
            float red = (a.X == b.X) ? b.X : Ease(shape, extent, a.X, b.X, t);
            float green = (a.Y == b.Y) ? b.Y : Ease(shape, extent, a.Y, b.Y, t);
            float blue = (a.Z == b.Z) ? b.Z : Ease(shape, extent, a.Z, b.Z, t);
            float alpha = (a.W == b.W) ? b.W : Ease(shape, extent, a.W, b.W, t);
        ...
        static float EaseOutElastic(float a, float b, float t)
        {
            ...
            float i = b;
            float s = p / 4f;
            return (i * (float)Math.Pow(2f, -10f * t) * (float)Math.Sin((t * 1f - s) * (2f * (float)Math.PI) / p) + b +
                    a);
        }
        ...
        static float EaseInOutElastic(float a, float b, float t)
        {
            ...
            if (t < 1)
            {
                return -.5f * (i * (float)Math.Pow(2f, 10f * (t -= 1f)) *
                               (float)Math.Sin((t * 1f - s) * (2f * (float)Math.PI) / p)) + a;
            }

            return i * (float)Math.Pow(2f, -10f * (t -= 1f)) *
                (float)Math.Sin((t * 1f - s) * (2f * (float)Math.PI) / p) *
                0.5f + b + a;
        }
